...
const StockPage: React.FC = () => {
  ...
  const setEditArticleStringFieldState = (
    fieldName: string,
    newValue: string,
  ) => {
    setState((prev) => ({
  ...
  const getUsers = async () => {
    try {
      api("/api/user/", "get", {}, "administrator").then((res: ApiResponse) => {
        if (res.status === "login") {
          setIsLoggedInStatus(false);
          return;
        }
      ...
      setErrorMessage(
        "Greška prilikom dohvaćanja korisnika. Greška: " + error,
        "danger",
        "err-users",
      );
    }
  ...
  const editFeatureCategoryChanged = async () => {
    try {
      ...
      features.forEach((feature) => {
        const existingFeature = updatedFeatures.find(
      ...
      setErrorMessage(
        "Greška prilikom dohvatanja osobina. Greška: " + error,
        "danger",
        "err-edit-features",
      );
    }
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const features: FeatureBaseType[] = res.data.map((item: any) => ({
          ...
          stockFeatureId: item.stockFeatures.map(
            (feature: any) => feature.stockFeatureId,
          ),
        }));
        ...
        setErrorMessage(
          "Greška prilikom dohvatanja osobina po kategoriji. Greška: " + error,
          "danger",
          "err-get-features-id",
        );
      }
  ...
  const putArticleDetailsInState = async (article: StockType) => {
    try {
      ...
      setEditArticleStringFieldState(
        "description",
      ...
      setEditArticleStringFieldState(
        "valueOnConcract",
        String(article.valueOnContract),
      );
      setEditArticleStringFieldState(
        "valueAvailable",
        String(article.valueAvailable),
      );
  ...
  const doEditArticle = () => {
    ...
    api("api/stock/" + stockID, "put", requestBody, "administrator").then(
      (res: ApiResponse) => {
        if (res.status === "login") {
          setIsLoggedInStatus(false);
          return;
  ...
  const changeStatus = () => {
    try {
      ...
      setErrorMessage(
        "Greška prilikom izmjene status. Greška:" + err,
        "danger",
        "err-change-status",
      );
    }
      ...
      >
        ...
        >
          <Input
            ...
            onChange={(e) =>
              setEditFeatureValue(feature.featureId, e.target.value)
            }
  ...
  const printOptionalMessage = () => {
    ...
    return <Alert variant="danger" title="Info!" body={state.error.message} />;
  };
  ...
  const changeStatusButton = () => {
    ...
    if (status === 0) {
      return <Chip color="danger">nema na stanju</Chip>;
    }
    if (status > 0) {
      return (
        <div id="modal-zaduzenja">
          <Button size="sm" color="success" onClick={() => showModal()}>
            <i className="bi bi-pencil-square" /> Izmjeni/zaduži
          </Button>
          <Modal
            size="lg"
            backdrop="blur"
          ...
          >
            <ModalContent>
              <ModalHeader>Kartica zaduženja</ModalHeader>
              <ModalBody>
                <Select
                  variant="bordered"
                  label="Status"
                ...
                <Input
                  ...
                  labelPlacement="inside"
                  onChange={(e) =>
                    setChangeStatusStringFieldState("invNumber", e.target.value)
                  }
                />
                ...
                />
                <ModalFooter>
                  <Button color="success" onClick={() => changeStatus()}>
                    {" "}
                    Sačuvaj{" "}
                  </Button>
                </ModalFooter>
  ...
  const renderStockData = (article: StockType) => {
    return (
      <>
        <div className="lg:flex">
              ...
              <div className="xs:w-full lg:w-8/12">
                {article.stockFeatures && article.stockFeatures[0] && (
                  <ScrollShadow hideScrollBar className="h-[180px]">
                    <Listbox
                      variant="flat"
                      key={`stockFeatures:${article.stockFeatures[0].featureId}`}
                      aria-label="Box specifikacije"
                    >
                      {article.stockFeatures.map((feature, index) => (
                        <ListboxItem
                          textValue={`nadogradnja-${index}`}
                          key={index}
                        >
                          {feature.feature && <b>{feature.feature.name}:</b>}{" "}
                          {feature.value}
                        </ListboxItem>
                      ))}
                    </Listbox>
          ...
          <div className="w-full sm:w-full lg:w-1/3">
            <Card className="shadow">
              ...
              <Listbox
                aria-label="Odaberi stavku iz liste"
                variant="flat"
                key={`stock:${article.stockId}`}
              >
                <ListboxItem
                  textValue={"Stanje po ugovoru"}
                  key={"stanje-po-ugovoru"}
                >
                  Stanje po ugovoru: {article.valueOnContract}
                </ListboxItem>
                <ListboxItem
                  textValue={"Dostupno stanje"}
                  key={"dostupno-stanje"}
                >
                  Trenutno stanje: {article.valueAvailable}
                </ListboxItem>
                <ListboxItem textValue={"SAP broj"} key={"sap-broj"}>
                  SAP broj: {article.sapNumber}
                </ListboxItem>
                <ListboxItem
        ...
        <div id="tabela-artikala" className="mt-5 w-full">
          {" "}
          {/* drugi kontejner punog sadržaja */}
        ...
        <Modal
          ...
          backdrop="blur"
          isOpen={state.editFeature.visible}
          onClose={() => setEditFeatureModalVisibleState(false)}
          ...
          <ModalContent id="modal-edit-sadrzaj">
            ...
            <ModalBody id="modal-edit-body">
              <ScrollShadow
                id="sjena-tekst"
                hideScrollBar
                className="grid max-h-[110vh] w-full gap-3"
              >
                <Input
                  type="text"
                  variant="bordered"
                  ...
                  onChange={(e) =>
                    setEditArticleStringFieldState("name", e.target.value)
                  }
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setEditArticleStringFieldState("excerpt", e.target.value)
                  }
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditArticleStringFieldState("concract", e.target.value)
                  }
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditArticleNumberFieldState(
                      "valueOnConcract",
                      e.target.value,
                    )
                  }
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditArticleStringFieldState("sapNumber", e.target.value)
                  }
            ...
            <ModalFooter id="modal-edit-footer">
              <Button color="success" onClick={() => doEditArticle()}>
                {" "}
                Sačuvaj
              </Button>
            </ModalFooter>
        ...
        {loading ? (
          ...
          <Card>
              ...
              <div className="flex w-full items-center justify-between rounded-xl bg-default-100 p-2">
                <div className="flex items-center">
                  ...
                  <div className="pl-2 text-left">
                    {state.stock ? state.stock?.name : "Oprema nije pronađena"}
                  </div>
                </div>
                <div className="flex items-center">
                  {badgeStatus()}
                  <Button
                    ...
                    color="success"
                    variant="shadow"
                    onClick={() => showEditFeatureModal()}
                  >
                    <i className="bi bi-pencil-square" /> Izmjeni
                  </Button>
                </div>
              </div>
            </CardHeader>
            <CardBody>
              ...
              {state.stock ? renderStockData(state.stock) : ""}
            </CardBody>
          </Card>
        )}
      </div>
