...
const NewTicketWithoutArticle: React.FC<ModalProps> = ({show, onHide, data}) => {
    ...
    const handleGroupTypeChange = (value: React.ChangeEvent<HTMLSelectElement>) => {
                ...
                <ModalContent>
                    ...
                    {selectedGroup ? (
                        <Select
                            ...
                            selectedKeys={addNewTicketState?.groupPartentId ? [`${addNewTicketState.groupPartentId}`] : []}
                        >
                            {groupsTypeState ? groupsTypeState
                                .map((group) => (
                                    <SelectItem 
                    ...
                    <Select 
                        label="Oprema"
                        labelPlacement='inside'
                        value={addNewTicketState?.articleId === null ? '' : addNewTicketState?.articleId}
                        selectedKeys={addNewTicketState?.articleId ? [`${addNewTicketState?.articleId}`] : []}
                        ...
                        >
                            ...
                            >
                                <div className="flex gap-2 items-center">
                                            <div className="flex flex-col">
                                                <span className="text-small">{article?.stock?.name}</span>
                                                <span className="text-tiny text-default-400">{article.category?.name}</span>
                                            </div>
                                        </div>
